...
public class Record {
    ...
    public void reset(ContO[] aconto) {
        ...
        do {
            if (this.prepit) {
                this.starcar[i] = new ContO(aconto[i], 0, 0, 0, 0, this.cp);
            }
            this.fix[i] = -1;
            this.dest[i] = -1;
            this.cntdest[i] = 0;
        ...
        do {
            int j = 0;
            do {
                this.car[i][j] = new ContO(aconto[j], 0, 0, 0, 0, this.cp);
                this.squash[i][j] = 0;
            } while (++j < 7);
        } while (++i < 6);
        i = 0;
    ...
    public Record(Medium medium, CheckPoints checkpoints) {
        this.ocar = new ContO[7];
        this.car = new ContO[6][7];
        this.squash = new int[6][7];
        this.fix = new int[7];
        this.dest = new int[7];
        this.x = new int[600][7];
        this.y = new int[600][7];
        this.z = new int[600][7];
        this.xy = new int[600][7];
        this.zy = new int[600][7];
        this.xz = new int[600][7];
        this.wxz = new int[600][7];
        this.wzy = new int[600][7];
        this.ns = new int[7][4];
        ...
        this.checkpoint = new int[600];
        this.lastcheck = new boolean[600];
        this.starcar = new ContO[7];
        this.hsquash = new int[7];
        this.hx = new int[600][7];
        this.hy = new int[600][7];
        this.hz = new int[600][7];
        this.hxy = new int[600][7];
        this.hzy = new int[600][7];
        this.hxz = new int[600][7];
        this.hwxz = new int[600][7];
        this.hwzy = new int[600][7];
        ...
        this.hcheckpoint = new int[600];
        this.hlastcheck = new boolean[600];
        this.cntdest = new int[7];
        this.m = medium;
        this.cp = checkpoints;
        this.caught = 0;
        this.cotchinow(0, checkpoints);
    }
    ...
    public void rec(ContO conto, int i, int j, int k, int l) {
        ...
        if (this.cntf == 50) {
            int i1 = 0;
            do {
                this.car[i1][i] = new ContO(this.car[i1 + 1][i], 0, 0, 0, 0, this.cp);
                this.squash[i1][i] = this.squash[i1 + 1][i];
            } while (++i1 < 5);
            this.car[5][i] = new ContO(conto, 0, 0, 0, 0, this.cp);
            this.squash[5][i] = j;
            this.cntf = 0;
        } else {
            ++this.cntf;
        ...
        if (this.dest[i] == 230) {
            if (i == 0) {
                this.cotchinow(0, this.cp);
                this.whenwasted = 229;
            } else if (k != 0) {
                this.cotchinow(i, this.cp);
        ...
        do {
            ...
            int k1 = 0;
            do {
                int[] nArray = this.sspark[i][j1];
                int n4 = k1++;
                nArray[n4] = nArray[n4] - 1;
            } while (k1 < 30);
            k1 = 0;
            do {
                int[] nArray = this.ry[i][j1];
                int n5 = k1;
                nArray[n5] = nArray[n5] - 1;
                int[] nArray2 = this.rx[i][j1];
                int n6 = k1;
                nArray2[n6] = nArray2[n6] - 1;
                int[] nArray3 = this.rz[i][j1];
                int n7 = k1++;
    ...
    public void recx(int i, float f, int j) {
        this.rx[j][i][this.nry[j][i]] = 600;
        this.magx[j][i][this.nry[j][i]] = (int)f;
        int[] nArray = this.nrx[j];
        int n = i;
        nArray[n] = nArray[n] + 1;
        if (this.nrx[j][i] == 7) {
            this.nrx[j][i] = 0;
    ...
    public void cotchinow(int i, CheckPoints checkpoints) {
        if (this.caught >= 600 && checkpoints.stage != 18 && checkpoints.stage != 24 && checkpoints.stage != 27 && checkpoints.stage != 29 && checkpoints.stage != 34 && checkpoints.stage != 41 && checkpoints.stage != 42 && checkpoints.stage != 55 && checkpoints.stage != 59 && checkpoints.stage != 60) {
            this.wasted = i;
            int j = 0;
            do {
                this.starcar[j] = new ContO(this.car[0][j], 0, 0, 0, 0, checkpoints);
    ...
    public void recz(int i, float f, int j) {
        this.rz[j][i][this.nry[j][i]] = 600;
        this.magz[j][i][this.nry[j][i]] = (int)f;
        int[] nArray = this.nrz[j];
        int n = i;
        nArray[n] = nArray[n] + 1;
        if (this.nrz[j][i] == 7) {
            this.nrz[j][i] = 0;
