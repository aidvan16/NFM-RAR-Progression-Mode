...
implements Runnable {
    ...
    public int readcookie(String s) {
        try {
            BufferedReader br = new BufferedReader(new FileReader(s + ".dat"));
    ...
    public void paint(Graphics graphics) {
        graphics.drawImage(this.offImage, 0, 0, this.getWidth(), this.getHeight(), this);
    }

    public GameSparker() {
        this.loadet = new boolean[19];
    }

    public void loadbase(ContO[] aconto, Medium medium, Trackers trackers, CheckPoints checkpoints, xtGraphics xtgraphics) {
        String[] as = new String[]{"2000tornados", "formula7", "canyenaro", "lescrab", "nimi", "maxrevenge", "leadoxide", "koolkat", "drifter", "policecops", "mustang", "king", "audir8", "masheen", "radicalone", "drmonster", "basicracer", "turbodragster", "deserthumvee", "lamborghinigallardo", "corvette", "radicalracer", "saleens7twinturbo", "stingrod", "zonichtank", "matlostank", "turbotank", "rebound", "rocketking", "rocketmasheen", "rocketmonster", "awesomeradicalone", "overkill", "phantom", "lightning", "killomatic", "destroyer", "train", "warhead", "tacticalnuke", "road", "froad", "twister2", "twister1", "turn", "offroad", "bumproad", "offturn", "nroad", "nturn", "roblend", "noblend", "rnblend", "roadend", "offroadend", "hpground", "ramp30", "cramp35", "dramp15", "dhilo15", "slide10", "takeoff", "sramp22", "offbump", "offramp", "sofframp", "halfpipe", "spikes", "rail", "thewall", "checkpoint", "fixpoint", "offcheckpoint", "sideoff", "bsideoff", "uprise", "riseroad", "sroad", "teleporter", "roughterrain", "wall", "smoothterrain", "tunnel", "tree", "cloud", "mountain", "hugeramp", "moon", "enableflying"};
        xtgraphics.dnload += 6;
        try {
            ...
            while (zipentry != null) {
                ...
                while (k < abyte0.length) {
                    l = zipinputstream.read(abyte0, k, abyte0.length - k);
                ...
                aconto[i] = new ContO(abyte0, medium, trackers, checkpoints, 0, 0, 0);
                ++xtgraphics.dnload;
    ...
    public boolean mouseDown(Event event, int n, int n2) {
        if (!this.exwist && this.mouses == 0) {
            this.xm = n * 670 / this.getWidth();
            this.ym = n2 * 400 / this.getHeight();
            this.mouses = 1;
        }
        return false;
    }
    ...
    public void loadstage(ContO[] aconto, ContO[] aconto1, Medium medium, Trackers trackers, CheckPoints checkpoints, xtGraphics xtgraphics, Madness[] amadness, Record record) {
        ...
        try {
            String s;
            URL url = new URL(this.getCodeBase(), "stages/" + checkpoints.stage + ".txt");
            DataInputStream datainputstream = new DataInputStream(url.openStream());
            while ((s = datainputstream.readLine()) != null) {
                ...
                if (s1.startsWith("set")) {
                    ...
                    aconto[this.nob] = new ContO(aconto1[k1 += 30], this.getint("set", s1, 1), this.getint("set", s1, 3), this.getint("set", s1, 2), this.getint("set", s1, 4), this.cp);
                    if (s1.indexOf(")p") != -1) {
                ...
                if (s1.startsWith("chk")) {
                    ...
                    aconto[this.nob] = new ContO(aconto1[l1 += 30], this.getint("chk", s1, 1), this.getint("chk", s1, 3), this.getint("chk", s1, 2), this.getint("chk", s1, 4), this.cp);
                    checkpoints.x[checkpoints.n] = this.getint("chk", s1, 1);
                    checkpoints.z[checkpoints.n] = this.getint("chk", s1, 2);
                    checkpoints.y[checkpoints.n] = this.getint("chk", s1, 3);
                    checkpoints.typ[checkpoints.n] = this.getint("chk", s1, 4) == 0 ? 1 : 2;
                ...
                if (s1.startsWith("fix")) {
                    ...
                    aconto[this.nob] = new ContO(aconto1[i2 += 30], this.getint("fix", s1, 1), this.getint("fix", s1, 3), this.getint("fix", s1, 2), this.getint("fix", s1, 4), this.cp);
                ...
                if (s1.startsWith("maxr")) {
                    ...
                    for (int j5 = 0; j5 < j2; ++j5) {
                        aconto[this.nob] = new ContO(aconto1[69], j3, medium.ground - aconto1[69].grat, j5 * 4800 + j4, 0, this.cp);
                        ++this.nob;
                    }
                ...
                if (s1.startsWith("maxl")) {
                    ...
                    for (int k5 = 0; k5 < k2; ++k5) {
                        aconto[this.nob] = new ContO(aconto1[69], k3, medium.ground - aconto1[69].grat, k5 * 4800 + k4, 0, this.cp);
                        ++this.nob;
                    }
                    trackers.y[trackers.nt] = -5000;
                    trackers.rady[trackers.nt] = 7100;
                ...
                if (s1.startsWith("maxt")) {
                    ...
                    for (int l5 = 0; l5 < l2; ++l5) {
                        aconto[this.nob] = new ContO(aconto1[69], l5 * 4800 + l4, medium.ground - aconto1[69].grat, l3, 90, this.cp);
                        ++this.nob;
                    }
                    trackers.y[trackers.nt] = -5000;
                    trackers.rady[trackers.nt] = 7100;
                ...
                for (int i6 = 0; i6 < i3; ++i6) {
                    aconto[this.nob] = new ContO(aconto1[69], i6 * 4800 + i5, medium.ground - aconto1[69].grat, i4, 90, this.cp);
                    ++this.nob;
                }
                trackers.y[trackers.nt] = -5000;
                trackers.rady[trackers.nt] = 7100;
        ...
        j1 = 0;
        do {
            aconto[j1] = new ContO(aconto1[xtgraphics.sc[j1]], xtgraphics.xstart[j1], 250 - aconto1[xtgraphics.sc[j1]].grat, xtgraphics.zstart[j1], 0, this.cp);
    ...
    public void run() {
        ...
        this.TracksSetup(trackers);
        CheckPoints checkpoints = new CheckPoints(this.cp);
        ...
        xtGraphics xtgraphics = new xtGraphics(medium, checkpoints, this.rd, this);
        xtgraphics.loaddata(k);
        Record record = new Record(medium, checkpoints);
        ContO[] aconto = new ContO[89];
        this.loadbase(aconto, medium, trackers, checkpoints, xtgraphics);
        ContO[] aconto1 = new ContO[3000];
        Madness[] amadness = new Madness[7];
        Madness madness = new Madness(medium, record, xtgraphics, i);
        this.maxmo = 0;
        int l = 0;
        int[] ai99 = new int[600];
        for (int i1 = 0; i1 < this.maxco; ++i1) {
            if (!aconto1[i1].colides) continue;
            ai99[l] = i1;
            ++l;
        }
        int[] ai19 = new int[20];
        ...
        int savedCars = this.readcookie("carsunlocked");

        if (savedCars >= 5 && savedCars <= 40) {
            xtgraphics.carsUnlocked = savedCars;
        } else {
            xtgraphics.carsUnlocked = 5;
        }
        
        int savedCar = this.readcookie("usercar");
        int savedCarPos = xtgraphics.getCarOrderIndex(savedCar);

        if (savedCar >= 0 &&
            savedCar <= 39 &&
            savedCarPos >= 0 &&
            savedCarPos < xtgraphics.carsUnlocked &&
        (savedCar != 33 || xtgraphics.carsUnlocked >= 40)) {

            xtgraphics.sc[0] = savedCar;
        } else {
            xtgraphics.sc[0] = xtgraphics.carorder[0];
        }   
        ...
        // DEBUG TRACE STATE - temporary
        int debugLastFase = Integer.MIN_VALUE;
        int debugLastStage = Integer.MIN_VALUE;
        int debugLastUnlocked = Integer.MIN_VALUE;
        int debugFinishCalls = 0;

        this.exwist = false;
        System.out.println("[DBG-GS] entering main run loop"
                + " stage=" + checkpoints.stage
                + " unlocked=" + xtgraphics.unlocked
                + " carsUnlocked=" + xtgraphics.carsUnlocked
                + " fase=" + xtgraphics.fase);

        while (true) {
            long l2;

            if (xtgraphics.fase != debugLastFase
                    || checkpoints.stage != debugLastStage
                    || xtgraphics.unlocked != debugLastUnlocked) {

                System.out.println("[DBG-GS] STATE CHANGE"
                        + " fase=" + debugLastFase + " -> " + xtgraphics.fase
                        + " stage=" + debugLastStage + " -> " + checkpoints.stage
                        + " unlocked=" + debugLastUnlocked + " -> " + xtgraphics.unlocked
                        + " carsUnlocked=" + xtgraphics.carsUnlocked
                        + " winner=" + xtgraphics.winner
                        + " flag=" + flag
                        + " k1=" + k1);

                debugLastFase = xtgraphics.fase;
                debugLastStage = checkpoints.stage;
                debugLastUnlocked = xtgraphics.unlocked;
            }
            Date date1 = new Date();
            ...
            if (xtgraphics.fase == -5) {
                    ...
                    ++debugFinishCalls;
                    boolean debugPrintResult = debugFinishCalls <= 5 || debugFinishCalls % 60 == 0;
                    int completedStage = checkpoints.stage;

                    if (debugPrintResult) {
                        System.out.println("[DBG-GS] FASE -5 ENTER call=" + debugFinishCalls
                                + " completedStage=" + completedStage
                                + " unlocked=" + xtgraphics.unlocked
                                + " carsUnlocked=" + xtgraphics.carsUnlocked
                                + " winner=" + xtgraphics.winner
                                + " flag=" + flag
                                + " enter=" + this.u[0].enter
                                + " handb=" + this.u[0].handb);
                        System.out.println("[DBG-GS] BEFORE xtgraphics.finish()");
                    }

                    xtgraphics.finish(checkpoints, aconto, this.u[0]);

                    if (debugPrintResult) {
                        System.out.println("[DBG-GS] AFTER xtgraphics.finish()"
                                + " fase=" + xtgraphics.fase
                                + " stage=" + checkpoints.stage
                                + " unlocked=" + xtgraphics.unlocked);
                    }

                    // finish() changes fase to 10 only when Continue is pressed.
                    if (flag && xtgraphics.fase == 10) {
                        System.out.println("[DBG-GS] RESULTS CONTINUE detected; BEFORE progression update"
                                + " completedStage=" + completedStage
                                + " unlocked=" + xtgraphics.unlocked
                                + " carsUnlocked=" + xtgraphics.carsUnlocked
                                + " winner=" + xtgraphics.winner);

                        if (completedStage == xtgraphics.unlocked && xtgraphics.winner) {

                            // Unlock a car after every even-numbered stage
                            if (completedStage % 2 == 0 && xtgraphics.carsUnlocked < 40) {
                                ++xtgraphics.carsUnlocked;
                                System.out.println("[DBG-GS] CAR UNLOCK incremented carsUnlocked -> "
                                        + xtgraphics.carsUnlocked);
                            }

                            // Unlock next stage
                            if (xtgraphics.unlocked < 70) {
                                ++xtgraphics.unlocked;

                                // Move the stage selector forward as well
                                checkpoints.stage = xtgraphics.unlocked;

                                System.out.println("[DBG-GS] STAGE UNLOCK incremented unlocked -> "
                                        + xtgraphics.unlocked
                                        + "; checkpoints.stage -> " + checkpoints.stage);
                            }
                        } else {
                            System.out.println("[DBG-GS] progression condition FALSE:"
                                    + " completedStage=" + completedStage
                                    + " unlocked=" + xtgraphics.unlocked
                                    + " winner=" + xtgraphics.winner);
                        }

                        System.out.println("[DBG-GS] BEFORE progression saves");
                        this.savecookie("unlocked", "" + xtgraphics.unlocked);
                        this.savecookie("carsunlocked", "" + xtgraphics.carsUnlocked);
                        this.savecookie("gameprfact", "" + (int)f);
                        this.savecookie("usercar", "" + xtgraphics.sc[0]);
                        System.out.println("[DBG-GS] AFTER progression saves");

                        flag = false;
                        System.out.println("[DBG-GS] flag reset FALSE");
                    }

                    if (debugPrintResult) {
                        System.out.println("[DBG-GS] BEFORE ctachm() from fase -5");
                    }
                    xtgraphics.ctachm(this.xm, this.ym, this.mouses, this.u[0]);
                    if (debugPrintResult) {
                        System.out.println("[DBG-GS] AFTER ctachm() from fase -5");
                    }

                if (checkpoints.stage == 70 && xtgraphics.winner) {
                    this.catchlink(1);
                }
                if (this.mouses == 2) {
                    this.mouses = 0;
            ...
            if (xtgraphics.fase == 5) {
                ...
                if (!flag) {
                    System.out.println("[DBG-GS] fase=5 BEFORE initial usercar save; setting flag afterwards");
                    this.savecookie("usercar", "" + xtgraphics.sc[0]);
                    flag = true;
                    System.out.println("[DBG-GS] fase=5 flag is now TRUE");
                }
            }
            ...
            if (xtgraphics.fase == 3) {
                xtgraphics.loadingfailed(checkpoints.stage, this.u[0], checkpoints);
                xtgraphics.ctachm(this.xm, this.ym, this.mouses, this.u[0]);
                if (this.mouses == 2) {
                    this.mouses = 0;
                }
            ...
            if (xtgraphics.fase == 12) {

    // Silent stage reload for the stage selector.
    // Temporarily use fase 2 so loadstage() performs its normal setup,
    // but DO NOT call loadingstage().
                xtgraphics.fase = 2;
            ...
            if (xtgraphics.fase == 176) {
                ...
                medium.follow(aconto1[0], madness, 0, 0);
            ...
            if (xtgraphics.fase == 0) {
                ...
                do {
                    ...
                    aconto1[k3] = new ContO(aconto[amadness[k3].cn], aconto1[k3].x, aconto1[k3].y, aconto1[k3].z, 0, this.cp);
                    aconto1[k3].xz = j5;
                    aconto1[k3].xy = j6;
                    aconto1[k3].zy = l8;
                ...
                if (xtgraphics.starcnt == 0) {
                    ...
                    do {
                        ...
                        do {
                            if (j14 == l12) continue;
                            amadness[l12].colide(aconto1[l12], amadness[j14], aconto1[j14], xtgraphics);
                        } while (++j14 < 7);
                    } while (++l12 < 7);
                    l12 = 0;
                    do {
                        amadness[l12].drive(this.u[l12], aconto1[l12], trackers, checkpoints, aconto1, ai19, this.maxmo);
                    } while (++l12 < 7);
                    l12 = 0;
                    do {
                        record.rec(aconto1[l12], l12, amadness[l12].squash, amadness[l12].lastcolido, amadness[l12].cntdest);
                    } while (++l12 < 7);
                    checkpoints.checkstat(amadness, aconto1, record, checkpoints);
                    l12 = 1;
                    do {
                        this.u[l12].preform(amadness[l12], aconto1[l12], checkpoints, trackers);
                    } while (++l12 < 7);
                    int j29 = 0;
                    int[] ai3 = new int[this.maxco];
                    for (int j3 = 0; j3 < this.maxco; ++j3) {
                        if (aconto1[j3].dist != 0) {
                            ai3[j29] = j3;
                            ++j29;
                            continue;
                        }
                        aconto1[j3].d(this.rd);
                    }
                    int[] ai5 = new int[j29];
                    for (int i4 = 0; i4 < j29; ++i4) {
                        ai5[i4] = 0;
                        for (int k6 = 0; k6 < j29; ++k6) {
                            if (aconto1[ai3[i4]].dist != aconto1[ai3[k6]].dist) {
                                if (aconto1[ai3[i4]].dist >= aconto1[ai3[k6]].dist) continue;
                                int n = i4;
                                ai5[n] = ai5[n] + 1;
                                continue;
                            }
                    ...
                    for (int j4 = 0; j4 < j29; ++j4) {
                        for (int l6 = 0; l6 < j29; ++l6) {
                            if (ai5[l6] != j4) continue;
                            if (aconto1[ai3[l6]].fire && ai3[l6] == ai19[0]) {
                                madness.dl(this.rd);
                            }
                            aconto1[ai3[l6]].d(this.rd);
                        }
                    }
                    if (xtgraphics.fase == 0) {
                        for (int k4 = 0; k4 < l; ++k4) {
                            for (int i7 = 0; i7 < this.maxmo; ++i7) {
                                if (ai19[i7] == ai99[k4]) continue;
                                aconto1[ai99[k4]].tryexp(aconto1[ai19[i7]]);
                                if (!aconto1[ai19[i7]].fire || i7 != 0) continue;
                                madness.lasercolid(aconto1[ai99[k4]]);
                            }
                        }
                    } else {
                        for (int i5 = l - 1; i5 >= 0; --i5) {
                            for (int j7 = 0; j7 < this.maxmo; ++j7) {
                                if (ai19[j7] == ai99[i5]) continue;
                                if (j7 != 1) {
                                    aconto1[ai99[i5]].tryexp(aconto1[ai19[j7]]);
                                }
                                if (!aconto1[ai19[j7]].fire || j7 != 0) continue;
                                madness.lasercolid(aconto1[ai99[i5]]);
                            }
                        }
                    }
                    for (int j5 = 1; j5 < this.maxmo; ++j5) {
                        if (ai29[j5] == 0) {
                            if (aconto1[ai19[j5]].exp) {
                                if (!this.nosound) {
                                    // empty if block
                                }
                                ai29[j5] = -1;
                            }
                            if (aconto1[ai19[j5]].hit && !this.nosound && this.frags == 0 && this.sosun) {
                                this.frags = 3;
                            }
                        }
                        if (ai29[j5] != 1) continue;
                        if (aconto1[ai19[j5]].exp) {
                            if (!this.nosound) {
                                // empty if block
                            }
                        ...
                        if (!aconto1[ai19[j5]].hit || this.nosound || this.frags != 0 || !this.sosun) continue;
                        this.frags = 3;
                    }
                } else {
                ...
                if (xtgraphics.starcnt < 38) {
                    if (this.view == 0) {
                        medium.follow(aconto1[0], madness, amadness[0].cxz, this.u[0].lookback);
                    ...
                    if (xtgraphics.starcnt == 38) {
                        ...
                        for (int dbg = 0; dbg < checkpoints.n; ++dbg) {
                            if (checkpoints.typ[dbg] > 0) {
                                ++dbgPositive;
                            }
                        }

                        System.out.println(
                            "[DBG-STAGE] BEFORE checkstat stage=" + checkpoints.stage +
                            " n=" + checkpoints.n +
                            " nsp=" + checkpoints.nsp +
                            " pcs=" + checkpoints.pcs +
                            " pcleared=" + amadness[0].pcleared +
                            " positivePoints=" + dbgPositive
                        );
                        checkpoints.checkstat(amadness, aconto1, record, this.cp);
                        System.out.println("[DBG-STAGE] AFTER checkstat");
                        medium.follow(aconto1[0], madness, amadness[0].cxz, 0);
                        xtgraphics.stat(amadness[0], checkpoints, this.u[0], true);
                        this.rd.setColor(new Color(255, 255, 255));
                        this.rd.fillRect(0, 0, 670, 400);
                    }
            ...
            if (xtgraphics.fase == -1) {
                if (k1 == 0) {
                    int i4 = 0;
                    do {
                        record.ocar[i4] = new ContO(aconto1[i4], 0, 0, 0, 0, this.cp);
                        aconto1[i4] = new ContO(record.car[0][i4], 0, 0, 0, 0, this.cp);
                    } while (++i4 < 7);
                }
                medium.d(this.rd);
                int j4 = 0;
                int[] ai3 = new int[this.nob];
                ...
                do {
                    ...
                    if (aconto1[l9].fcnt == 7 || aconto1[l9].fcnt == 8) {
                        aconto1[l9] = new ContO(aconto[amadness[l9].cn], 0, 0, 0, 0, this.cp);
                        record.cntdest[l9] = 0;
                    }
                    if (k1 == 299) {
                        aconto1[l9] = new ContO(record.ocar[l9], 0, 0, 0, 0, this.cp);
                    }
            ...
            if (xtgraphics.fase == -3) {
                if (k1 == 0) {
                    ...
                    int k4 = 0;
                    do {
                        aconto1[k4] = new ContO(record.starcar[k4], 0, 0, 0, 0, this.cp);
                    } while (++k4 < 7);
                }
                medium.d(this.rd);
                int i5 = 0;
                int[] ai4 = new int[this.nob];
                ...
                do {
                    ...
                    if (aconto1[l10].fcnt == 7 || aconto1[l10].fcnt == 8) {
                        aconto1[l10] = new ContO(aconto[amadness[l10].cn], 0, 0, 0, 0, this.cp);
            ...
            if (xtgraphics.fase == -4) {
                System.out.println("[DBG-GS] FASE -4 tick"
                        + " k1=" + k1
                        + " stage=" + checkpoints.stage
                        + " winner=" + xtgraphics.winner);

                if (k1 <= 0) {
                    System.out.println("[DBG-GS] FASE -4 drawing mdness/dude");
                    this.rd.drawImage(xtgraphics.mdness, 224, 30, null);
                    this.rd.drawImage(xtgraphics.dude[0], 70, 10, null);
                    System.out.println("[DBG-GS] FASE -4 finished mdness/dude draw");
                }

                if (k1 >= 0) {
                    System.out.println("[DBG-GS] FASE -4 BEFORE fleximage k1=" + k1);
                    xtgraphics.fleximage(this.offImage, k1, checkpoints.stage);
                    System.out.println("[DBG-GS] FASE -4 AFTER fleximage k1=" + k1);
                }

                ++k1;

                if (checkpoints.stage == 70 && ++k1 == 10) {
                    System.out.println("[DBG-GS] FASE -4 old-stage-60 fast transition -> -5");
                    xtgraphics.fase = -5;
                }

                if (k1 == 12) {
                    System.out.println("[DBG-GS] FASE -4 k1==12 transition -> -5");
                    xtgraphics.fase = -5;
                }
            }
            if (xtgraphics.fase == -6) {
    ...
    public void savecookie(String s, String s1) {
        System.out.println("[DBG-GS] savecookie BEGIN name=" + s + " value=" + s1);
        try {
            System.out.println("[DBG-GS] savecookie BEFORE FileWriter: " + s + ".dat");
            PrintWriter out = new PrintWriter(new FileWriter(s + ".dat"));
            System.out.println("[DBG-GS] savecookie AFTER FileWriter: " + s + ".dat");

            out.println(s1);
            System.out.println("[DBG-GS] savecookie AFTER println: " + s);

            out.close();
            System.out.println("[DBG-GS] savecookie END success: " + s);
        }
        catch (Exception e) {
            System.out.println("[DBG-GS] savecookie FAILED " + s + ": " + e);
            e.printStackTrace();
        }
    }
